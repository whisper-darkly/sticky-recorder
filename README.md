...
# Stripchat with segment splitting (1 hour segments, glitch-free)
sticky-recorder --driver stripchat -s bob --segment-length 1h -o 'rec/{{.Source}}/{{.Recording.Year}}-{{.Recording.Month}}-{{.Recording.Day}}'
...
# JSON output format
sticky-recorder --output-format json -s alice -o /tmp/out
```
...
| `--output-format` | | `normal` | Output format: `normal`, `json` |
...
| `--domain` | | | Override platform domain URL |
| `--version` | `-V` | | Print version and exit |

### Segment Splitting

When `--segment-length` is set, sticky-recorder uses ffmpeg's built-in segment muxer (`-f segment`) to split recordings into fixed-duration segments. This keeps a single ffmpeg process and stream connection alive across all segments, producing clean splits with no audio glitches or content overlap.

Segments are written directly by ffmpeg using the output template with a `_%05d` suffix (e.g. `recording_00000.ts`, `recording_00001.ts`, ...). Real-time `SEGMENT START` and `SEGMENT FINISH` events are emitted as each segment opens and closes. The `--exec` command runs on each completed segment.

Without `--segment-length`, recordings are written to a single file.
...
The `--output-format` flag (or `STICKY_OUTPUT_FORMAT`) controls log and event output:

- **`normal`** (default) — human-readable timestamped lines:
  ```
  2026/02/18 22:01:31 [EVENT] SEGMENT START file=/path/to/file_00000.ts segment=0 recording=0
  2026/02/18 22:01:31 [INFO] recording to /path/to/file.ts
  ```

- **`json`** — compact JSON, one object per line:
  ```json
  {"event":"SEGMENT START","file":"/path/to/file_00000.ts","recording":"0","segment":"0","time":"2026-02-18T22:01:31-05:00"}
  {"level":"info","message":"recording to /path/to/file.ts","time":"2026-02-18T22:01:31-05:00"}
  ```

Both formats are written to the terminal and to the log file (if `--log` is set).

### Environment Variables

CLI flags take priority over environment variables.

`STICKY_DRIVER`, `STICKY_RESOLUTION`, `STICKY_FRAMERATE`, `STICKY_SEGMENT_LENGTH`, `STICKY_COOKIES`, `STICKY_USER_AGENT`, `STICKY_OUT`, `STICKY_LOG`, `STICKY_LOG_LEVEL`, `STICKY_OUTPUT_FORMAT`, `STICKY_RETRY_DELAY`, `STICKY_SEGMENT_TIMEOUT`, `STICKY_RECORDING_TIMEOUT`, `STICKY_CHECK_INTERVAL`
